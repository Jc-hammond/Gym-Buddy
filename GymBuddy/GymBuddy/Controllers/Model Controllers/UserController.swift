    ...
    func createUser(fullName: String, currentWeight: Int, targetWeight: Int, completion: @escaping (Result<User?, UserError>) -> Void) {
        fetchAppleUserReference { result in
            ...
            case .success(let reference):
                ...
                let date = Date()
                let user = User(photo: nil, fullName: fullName, currentWeights: [currentWeight], currentDates: [date], targetWeight: targetWeight, friends: [], friendRefs: nil, friendRequests: nil, friendRequestRefs: nil, workouts: [], appleUserRef: reference)
